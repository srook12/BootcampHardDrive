...
public class ProductDBFromFile extends HashMap implements Database {
	...
	private final static String FILE_NAME = "src/data/products.tx";	
	private static Logger log = Logger.getLogger("mywork.logging.ex1");
	BufferedReader br;
    ...
    public boolean open () {
        ...
        try{
            ...
            while((productLine = br.readLine())!= null) {
            	...
            	log.fine("Added the product:\t" + product);
            }
            
        }catch(Exception e){
        	log.warning("An exception occurred when trying to open the file!");
            successFlag = false;
            e.printStackTrace();
        }finally{
